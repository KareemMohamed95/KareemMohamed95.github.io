...
function addSqrtSymbol(){
  ...
  MQ.MathField(document.getElementById(lastFocusedMQ)).cmd("\\sqrt");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addNthRootSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("{\\left(\\right)}^{\\frac{١}{٣}}");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
} 
function addFracSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).cmd("\\frac");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
...
function addSinSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("جا\\left(\\right)");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addCosSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("جتا\\left(\\right)");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addTanSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("ظا\\left(\\right)");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addCosecSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("قتا\\left(\\right)");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addSecSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("قا\\left(\\right)");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addCotanSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("ظتا\\left(\\right)");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addSinSquareSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("{جا\\left(\\right)}^{٢}");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addCosSquareSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("{جتا\\left(\\right)}^{٢}");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addTanSquareSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("{ظا\\left(\\right)}^{٢}");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addCosecSquareSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("{قتا\\left(\\right)}^{٢}");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addSecSquareSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("{قا\\left(\\right)}^{٢}");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addCotanSquareSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("{ظتا\\left(\\right)}^{٢}");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
...
function addAlphaSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("\\alpha");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addBetaSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("\\beta");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addGammaSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("\\gamma");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addThetaSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("\\theta");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addEpsilonSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("\\epsilon");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addPiSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).cmd("ط");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addInfinitySymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).cmd("\\infty");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
...
function clearQuillKeyboard(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).latex("");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addCdotSign(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("\\cdot");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
function addPMSign(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).write("\\pm");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
}
...
function addPhiSymbol(){
 ...
 function addLeftSetSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).cmd("\\right\\}");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
 }
 function addRightSetSymbol(){
  if(lastFocusedMQ == null)return;
  MQ.MathField(document.getElementById(lastFocusedMQ)).cmd("\\left\\{");
  MQ.MathField(document.getElementById(lastFocusedMQ)).focus();
 }
...
Node.open(function(_) {
  _.keystroke = function(key, e, ctrlr) {
    ...
    case 'Right': ctrlr.moveLeft(); break;
    case 'Shift-Left': ctrlr.selectLeft(); break;
    case 'Ctrl-Left': break;

    case 'Left': ctrlr.moveRight(); break;
...
if (transformPropName) {
  scale = function(jQ, x, y, isSqrt = false) {
    if(isSqrt == false)jQ.css(transformPropName, 'scale('+x+','+y+')');
    else jQ.css(transformPropName, 'scale('+x+','+y+') rotateY(180deg)');
  };
}
...
LatexCmds['\u221a'] = P(MathCommand, function(_, super_) {
  ...
  _.reflow = function() {
    ...
    scale(block.prev(), 1, block.innerHeight()/+block.css('fontSize').slice(0,-2) - .1, true);
  };
});
