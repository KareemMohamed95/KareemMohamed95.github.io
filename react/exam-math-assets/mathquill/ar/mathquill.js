 ...
 function ArabicToEnglishLatex(latex) {
  ...
  for(var i = 0;i < latex.length;i++) {
    ...
    if(mappedWord.length == 0){
      let finalMappedWord = window.mqArabicMapping[latex[i]];
      if(!(finalMappedWord === undefined || finalMappedWord === null))newLatex += finalMappedWord;
      else newLatex += latex[i];
    }
    else {
      let finalMappedWord = window.mqArabicMapping[mappedWord];
      if(!(finalMappedWord === undefined || finalMappedWord === null))newLatex += finalMappedWord;
      i = j-1;
    }
  }
...
CharCmds['*'] = LatexCmds['\u00d7'] = LatexCmds.times = bind(BinaryOperator, '\\times ', '&times;', '[x]');
